...
## Overview
This repository contains the source code and technical documentation for analyzing the Amazon Books Reviews dataset (available at: https://www.kaggle.com/datasets/mohamedbakhet/amazon-books-reviews)[cite: 2]. The project engineers a distributed data processing pipeline using Apache Spark to handle over 3 GB of raw e-commerce data, encompassing approximately 3 million individual review records and 212,404 unique book titles spanning from 1996 to 2014[cite: 2]. 
...
* **State Management:** Utilized Spark's `.cache()` and `.unpersist()` methods to dynamically lock validated data partitions in active memory while flushing legacy data structures[cite: 1].
...
3. **Apache Spark Configuration:**
   This project requires a local installation of Apache Spark. Ensure that `SPARK_HOME` and Java dependencies are correctly configured in your system environment variables. The `findspark` library is used to locate the Spark installation at runtime[cite: 1].

4. **Data Placement:**
   Download the raw dataset from Kaggle: https://www.kaggle.com/datasets/mohamedbakhet/amazon-books-reviews
   Extract and place the files (`books_data.csv` and `books_rating.csv`) into a `dataset/` directory at the root of the project[cite: 1]. 

## Usage
Execute the Jupyter Notebook sequentially. The notebook is structured into distinct phases:
1. Environment initialization and Spark Session allocation[cite: 1].
2. ETL, memory pruning, and the execution of the Broadcast Join[cite: 1].
3. Exploratory Data Analysis (EDA) and Data Quality validation[cite: 1].
4. PySpark aggregations (exporting intermediate results to the `Result/` directory)[cite: 1].
5. Pandas and Seaborn visualization rendering[cite: 1].
